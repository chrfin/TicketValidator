...
typedef struct _THEMEPROPERTYVALUEHDR
{
    ...
    int     cbValue;            // count of bytes of value
    BYTE    pbValue[1];        // value data.
} THEMEPROPERTYVALUEHDR, *PTHEMEPROPERTYVALUEHDR;

//---------------------------------------------------------------------------
//
//  THEMEPROPERTYVALUEHDR::iType can be one of the following values (defined in vssym32.h):
//
//  iType           prgValue binary data type
//  -----           -------------------------
//  TMT_HBITMAP:    HBITMAP (note: On return of GetThemePropertyValues(), the 
//                                 caller owns this GDI handle and must free it w/ DeleteObject().)
//  TMT_ENUM        INT                    
//  TMT_STRING      LPWSTR, null-terminated
//  TMT_BOOL        BOOL
//  TMT_COLOR       COLORREF
//  TMT_MARGINS     MARGINS 
//  TMT_SIZE        INT 
...
//                      - Retrieves multiple theme property values of 
//                        from the currently active theme.
//
//  prgProperties       - Array of THEMEPROPERTYID records specifying 
//                        the properties whose values are requested.
//
//  cProperties         - Number of THEMEPROPERTYID records contained
//                        in the prgProperties array.
//                    
//  ppbValues           - On function return, points at a buffer containing
//                        the requested property values.   Each value in
//                        the buffer consists of a THEMEPROPERTYVALUEHDR
//                        block followed by the value data.   String values
//                        consist of a NULL-terminated array of UNICODE characters.
//                        The buffer should be freed using FreeThemeValues().
//                        
//  pcbValues           - On function return, contains the total size, in bytes of 
//                        the buffer pointed to by ppvValues.
//
THEMEAPI GetThemePropertyValues( 
    IN const THEMEPROPERTYID* prgProperties, IN int cProperties, 
    OUT BYTE** ppbValues, OUT ULONG* pcbValues );

//---------------------------------------------------------------------------
//  FreeThemePropertyValues()
//
//                      - Frees the memory buffer obtained from GetThemeValues()
//
//  pbValues            - Value buffer obtained from GetThemePropertyValues.
//
...
typedef HANDLE HTHEMESYMBOLS;  // handle to theme symbol table

//---------------------------------------------------------------------------
//  LoadThemeSymbols()  - Load symbols for the current or specified theme.
//
//  pszThemeFile        - Full path name of theme file.  If NULL, symbols for 
//                        the currently active theme file are loaded.
//  phSymbols           - Address to receive the theme's symbol table handle.
//---------------------------------------------------------------------------
THEMEAPI LoadThemeSymbols( IN OPTIONAL LPCWSTR pszThemeFile, OUT HTHEMESYMBOLS* phSymbols );


//---------------------------------------------------------------------------
//  FreeThemeSymbols()  - Releases the symbol table.
//  
//  hSymbols            - symbol table handle to release.   When the function
//                        returns, the handle is invalid and should no longer be used.
//---------------------------------------------------------------------------
THEMEAPI FreeThemeSymbols( IN HTHEMESYMBOLS hSymbols );

//---------------------------------------------------------------------------
//  ParseThemeSymbolSpec() - Parses the specified theme symbol spec into 
//                           class, part, state and property components.
//
//  pszSpec                - Theme symbol specification in the form
//                           class.part.state.property.
//  
//  pszClass               - Address of string buffer to receive class name
//                           If NULL, the class name
//
//  pcchClass              - IN: length of pszClass buffer, in characters.   
//                         - OUT: number of characters copied to pszClass.
//                                If pszClass is NULL, pcchClass will receive
//                                the minimum buffer size, in characters, for 
//                                the class name.
//
//  piPart                 - if non-NULL, receives the part index
//
//  piState                - if non-NULL, receives the state index
//
//  piProperty             - if non-NULL, receives the property index
//
//  piType                 - if non-NULL, receives the property type
//---------------------------------------------------------------------------
THEMEAPI ParseThemeSymbol( 
    IN HTHEMESYMBOLS hThemeSymbol,
    IN LPCWSTR pszSpec,
    OUT OPTIONAL LPWSTR pszClass,
    IN OUT OPTIONAL int* pcchClass,
    OUT OPTIONAL int* piPart,
    OUT OPTIONAL int* piState,
    OUT OPTIONAL int* piProperty,
    OUT OPTIONAL int* piType );
